...
class ReceiptCalculatorTest {
    ...
    private final ReceiptCalculator calculator = new ReceiptCalculator();

    @BeforeEach
    void init() {
        receipt.setDiscountCard(DiscountCard.newBuilder().setDiscountPercentage(10).build());
    ...
    @Test
    void getTotalCost() {
        assertEquals(5.55,calculator.getTotalCost(receipt),0.1);
    }

    @Test
    void getTotalCostWithDiscount() {
        assertEquals(4.99,calculator.getTotalCostWithDiscount(receipt),0.1);
